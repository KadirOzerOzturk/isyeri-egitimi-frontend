...
function CompanyLogin() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (error.response.data === "Company not found" || error.response.data === "Password is not correct") {
        ...
        <div  className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
          <div  className="bg-slate-100  py-8 px-6 shadow rounded-lg sm:px-10">
            <form  className="mb-0 space-y-6" onSubmit={handleLogin}>
              <div>
                <label htmlFor="companyNumber"  className="block text-sm font-medium text-gray-700">Firma Numarası</label>
                <div  className="mt-1">
                  <input
