...
function LecturerLogin() {
 ...
 const navigate = useNavigate()
  ...
  const [izleyiciNo, setIzleyiciNo] = useState('');
  const [izleyiciParola, setIzleyiciParola] = useState('');
  ...
  const handleStudentNumberChange = (e) => {
    ...
    if (!isNaN(numericValue) || inputValue === '') {
      setIzleyiciNo(inputValue);
    }
  };

  const handlePasswordChange = (e) => {
    setIzleyiciParola(e.target.value);
  };
  ...
  const handleLogin = async (e) => {
    ...
    // } else {
    try {
      const res = await axios.post('/auth/lecturer-login', {
        izleyiciNo: izleyiciNo,
        izleyiciParola: izleyiciParola,
      });
      if (res.status === 200) {
        console.log("res" + res)
      ...
      if (error.response.data === "Lecturer not found" || error.response.data === "Password is not correct") {
        ...
        <div  className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
          <div  className="bg-slate-100  py-8 px-6 shadow rounded-lg sm:px-10">
            <form  className="mb-0 space-y-6" onSubmit={handleLogin}>
              <div>
                <label htmlFor="studentNumber"  className="block text-sm font-medium text-gray-700">İzleyici Numarası</label>
                <div  className="mt-1">
                  <input
